...
\begin{proof}
  ...
  Since $z_0 = q(y_0)$ and $q$ is an covering map, there exists $\beta$ such that $y_0 \in U_{y_{\beta}}$.
  ...
  \begin{itemize}
    ...
    \item
      ...
      Since $q \circ (p \circ (\widetilde{q \circ \gamma})) = (q \circ p) \circ (\widetilde{q \circ \gamma}) = q \circ \gamma$, $\gamma = p \circ (\widetilde{q \circ \gamma})$ by the uniqueness of a lift.
  ...
  \begin{align*}
    (q \mid_{U_{y_0}})^{-1} \circ (q \circ p) \mid_{U_{x_{\alpha}}}
      ...
      &= p \mid_{U_{x_{\alpha}}}
  \end{align*}
  Thus $p \mid_{U_{x_{\alpha}}}$ is a homeomorphism between $U_{x_{\alpha}}$ and $U_{y_0}$ because a composition of two homeomorphisms is a homeomorphism.
  ...
  Let $p_*([h]) \in p_*\pi_1(X)$ and $[g] \in \pi_1(Y)$.
...
\begin{proof}
  In this proof, we will invoke Proposition 1.39 without referring to it since it will be used so many times that the solution simply gets very long and less readable if we explicitly write it every time.
  ...
  \begin{itemize}
    \item
      Since $H$ is the commutator subgroup, $H$ is a normal subgroup of $\pi_1(X, x_0)$ by Lemma \ref{lem1}.
      Thus $N(H) = \pi_1(X, x_0)$.
  ...
  By Problem 16 from above, $\tilde{p}: \tilde{X} \rightarrow \tilde{X}'$ is a covering.
  Therefore, $\tilde{X}$ is a universal abelian covering.

  Let $q: Y \rightarrow X$ be a universal abelian covering space of $X$.
  ...
  \begin{itemize}
    \item
      ...
      $\tilde{X}$ is a normal covering because, for any grid point $(m, n)$, shifting $\tilde{X}$ by $m$ in the $x$ direction and $n$ in the $y$ direction gives the deck transformation mapping $(0, 0)$ to $(m, n)$.
...
\begin{proof}
  ...
  Therefore, $G(\tilde{M_g})$ is isomorphic to $\pi_1(M_g) / H$.
...
\begin{proof}
  ...
  $aHa^{-1} = \ev{a^3, aba^{-1}}$.
  Since $aba = b$, $\ev{a^3, aba^{-1}} = \ev{a^3, a^2b}$.
  ...
  $H = p_*\pi_1(\tilde{X})$ is generated by $a^3$ and $b^2a$ because the red and green lines in $\tilde{X}$ get mapped as in Figure \ref{fig:problem20_klein_by_torus}, and the green line in $X$ is homotopic to the path $ba$.
  Suppose $H$ is normal.
  ...
  On the other hand, $a^3, b^2a \in \ev{a, b^2}$, so $H = \ev{a, b^2}$.
  Since we assumed that $H$ is normal,
  \begin{align*}
    G(\tilde{X})
      &= \pi_1(X) / H \\
      &= \ev{a, b \mid abab^{-1}, a, b^2} \\
      &= \ev{b \mid b^2} \\
      &\cong \mathbb{Z}_2.
  \end{align*}
  This implies that $\abs{G(\tilde{X})} = 2$.
  However, $\tilde{X}$ is a 6-sheeted cover, $G(\tilde{X})$ must have 6 elements if it is normal.
  Therefore, $\tilde{X}$ is not normal.
\end{proof}
