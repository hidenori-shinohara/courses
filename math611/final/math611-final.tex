...
\begin{exer}{(Problem 6)}
  Let $g = 0$.
  Then $\pi_1(\Sigma_g) = 0$.
  By Proposition 1.32, there exists no connected, 3-sheeted cover.

  Let $g = 1$.
  Then $\Sigma_g$ is a torus.
  \begin{figure}[!htb]
    \includegraphics[width=.5\linewidth]{problem6_1.jpeg}
    \caption{$\Sigma_g$}
    \label{fig:problem6_1}
  \end{figure}
  Let $\tilde{\Sigma}_g$ be the covering space in Figure \ref{fig:problem6_1}.
  $\tilde{\Sigma}_g$ is a 3-sheeted covering space.
  The fundamental group of $\Sigma_g$ is $\mathbb{Z} \times \mathbb{Z}$, all of whose subgroups are normal.
  By Proposition 1.39, $\tilde{\Sigma_g}$ must be a normal covering space because $p_{*}(\pi_1(\tilde{X}))$ is normal.
  Therefore, when $g = 1$, $\Sigma_g$ does not have a connected, non-normal, 3-sheeted cover.
