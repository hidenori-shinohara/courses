...
\begin{proof}
  ...
  Let $\tilde{N_1}$ denote some open set in $\tilde{X}$ such that $p$ maps $\tilde{N_1}$ to $N_1$ homeomorphically.
  ...
  Then there exists $\tilde{x} \in \tilde{X}$ such that $\tilde{x} \in \tilde{N_i} \cap \tilde{V_{i + 1}}$ where $\tilde{V_{i + 1}}$ is some open set in $\tilde{X}$ such that $p$ maps $\tilde{V_{i + 1}}$ to $V_{i + 1}$ homeomorphically.
  Since $\tilde{N_i}$ and $\tilde{V_{i + 1}}$ are both open sets, the intersection $\tilde{V} = \tilde{N_i} \cap \tilde{V_{i + 1}}$ is a neighborhood of $\tilde{x}$.
  ...
  Let $N_{i + 1} = V_{i + 1} \cap N_i \cap ([0, \epsilon) \times [0, 1])$.
  Then this is an evenly covered neighborhood of the left edge, and let $\tilde{N_{i + 1}}$ be an open set in $\tilde{X}$ such that the restriction of $p$ is a homeomorphic.
  \todo[inline]{
    Explain this more.
    Why does this work?
  }
