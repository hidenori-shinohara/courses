...
\begin{proof}
  \todo[inline]{
    ...
    $f \circ g$ is homotopic to the identity map on $Y$.
    Then $f \circ g \circ p_Y$ is homotopic to $p_Y$.
    Let $F$ be a homotopy between them such that $F_0 = st$.
    This is possible because $st$ is clearly a lift of $f \circ g \circ p_Y$ if you look at the diagram.
    See Figure \ref{fig:problem8_idea}.
    We will look into what $F_1$ is.
    In the best case scenario, $F_1 = \Id_{\tilde{Y}}$, and $st$ is homotopic to $\Id_{\tilde{Y}}$.

    However, that might not be the case.
    Let $r = F_1$.
    Let $y_0 \in Y$ and $\tilde{y_0} \in Y$ be given such that $p_Y(\tilde{y_0}) = y_0$.
    Then $\tilde{y_0}, r(\tilde{y_0})$ are both points in $\tilde{Y}$.
    Let $k$ be a lift of $p_Y$ such that $k(r(\tilde{y_0})) = \tilde{y_0}$.
    $(p_Y \circ k) \circ r = p_Y \circ r = p_Y$.
    Therefore, $k \circ r$ is a lift of $p_Y$ such that $(k \circ r)(\tilde{y_0}) = \tilde{y_0}$.
    $\Id_{\tilde{y_0}}$ is such a lift as well.
    By the uniqueness of lifts, $k \circ r = \Id$.
    Then $k \circ s \circ t \simeq k \circ r = \Id$.
    Therefore, $(k \circ s) \circ t$ is homotopic to $\Id$, so $(k \circ s) \circ t$ is a homotopy equivalence.

    Similarly, there exists $k': \tilde{X} \rightarrow \tilde{X}$ such that $t \circ s \circ k' \simeq r' \circ k' = \Id_{\tilde{X}}$.
    Therefore, $t \circ (s \circ k')$ is a homotopy equivalence.

    By Problem 11 in Chapter 0, $t$ is a homotopy equivalence.
  }
  \begin{figure}
    \includegraphics[width=.5\linewidth]{problem8_idea.jpeg}
    \caption{Problem 8 idea}
    \label{fig:problem8_idea}
  \end{figure}
  \begin{figure}
