...
\begin{exer}{(Problem 5, Chapter 1.3)}
  ...
  Show that for every covering space $\tilde{X} \rightarrow X$ there is some neighborhood of the left edge of $X$ that lifts homeomorphically to $\tilde{X}$.
...
\begin{proof}
  ...
  Let $m \in \mathbb{N}$ be chosen such that the vertical line $x = 1 / m$ is in $N_k$.
  Let $\gamma$ be the loop that goes through $(0, 0), (1/m, 0), (1/m, 1), (0, 1)$.
  Then $[\gamma] \ne 0$ in $\pi_1(N_k)$.
  Let $\tilde{\gamma}$ be the lift of $\gamma$ in $\tilde{N}_k$.
  Note that $\tilde{\gamma}$ is a loop in $\tilde{X}$ because $p$ maps $\tilde{N}_k$ into $N_k$ homeomorphically.
  Since any path homotopy between $\tilde{\gamma}$ and another loop can be sent back to $X$, $\tilde{\gamma}$ cannot be homotopic to a constant loop.
