...
\begin{proof}
  ...
  \end{itemize}
  Therefore, $\ker(\Phi) = (x^2 - 2)$, so the homomorphism $\tilde{\Phi}$ induced by $\Phi$ is an isomorphism from $\mathbb{Z}/(p)[x]/(x^2 - 2) \rigtharrow \mathbb{Z}[\sqrt{2}]/(p)$ by the first isomorphism theorem.
\end{proof}
