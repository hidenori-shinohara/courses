  ...
  \begin{enumerate}[label=(\roman*)]
    ...
    \item
      \todo[inline,caption={}]{
        ...
        The idea is that if $f$ is irreducible, $K[x]/(f)$ is a field.
        Then consider $F: K[x]/(f) \rightarrow K[x]/(f)$.
        Then we have $\ker(\Id - F) = \{ g + (f) \mid (\Id - F)(g + (f)) = 0 \} = \{ g + (f) \mid g + (f) = F(g + (f)) \}$.
        But I'm not sure what this tells us.
        $t + (f) \notin \ker(\Id - F)$, but I don't know what to do with it.
        I wonder if this has anything to do with nilpotent elements?
        What happens if I take something like $(t + (f))^p$?
        A field can't have nilpotent elements.
      }
  \end{enumerate}
\end{proof}
